...
<div class="max-w-6xl mx-auto px-4 py-8 space-y-6">
    ...
    <section class="rounded-3xl bg-gradient-to-{{ $dir === 'rtl' ? 'l' : 'r' }} from-ink-950 via-ink-900 to-brand-800 text-white p-8 md:p-10 shadow-2xl text-center">
        <div class="text-brand-300 text-sm font-semibold">{{ __('Team of the Season') }} — {{ __('Official Results') }}</div>
                    ...
                    <div class="flex flex-wrap justify-center gap-3 md:gap-5">
                        @foreach($winnersBySlot->get($slot, collect())->sortBy('rank') as $item)
                            ...
                            <div class="w-40 rounded-2xl bg-white p-3 text-center shadow-lg">
                                <div class="w-20 h-20 mx-auto rounded-full bg-ink-100 overflow-hidden mb-2 flex items-center justify-center text-3xl">
                                    @if($photo)
                                ...
                                <div class="mt-2 text-xs text-brand-700 font-semibold">
